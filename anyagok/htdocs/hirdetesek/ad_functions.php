...
function list_ads(){
    ...
    . "FROM ads\n"
    . "INNER JOIN users ON ads.owner_id=users.id;";

    $conn = db_init();
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        echo "<table><tr><th>Termék neve</th><th>Termék leírása</th>"
        . "<th>Termék ára</th><th>Eladó neve</th><th>Eladó email címe</th></tr>";
        while($row = $result->fetch_assoc()) {
            echo "<tr>";
            echo "<td>" . $row["name"] . "</td>";
